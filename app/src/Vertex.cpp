...
Vertex::Vertex(const QVector3D &p, const QVector3D &pu, const QVector3D &pv, const QVector3D &n, float u, float v,
               double alpha, double beta) : position(p), puVector(pu), pvVector(pv), normal(n), rotatedPosition(p),
                                            rotatedPuVector(pu), rotatedPvVector(pv),
                                            rotatedNormal(n), u(u), v(v) {
